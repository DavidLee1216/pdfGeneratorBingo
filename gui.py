...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setTabs(self, MainWindow):
        ...
        self.tab_button_layout_widget.setGeometry(
            QtCore.QRect(80, 20, 770, self.tab_button_height))
        self.tab_button_layout_widget.setObjectName("tab_button_layout_widget")

        self.session_btn = PicButton(QtGui.QPixmap(
            './images/session_button.png'), self.tab_button_layout_widget)
        self.session_btn.clicked.connect(self.showSessionManager)

        self.book_btn = PicButton(QtGui.QPixmap(
            './images/generator_button.png'), self.tab_button_layout_widget)
        self.book_btn.clicked.connect(self.showBookGenerator)

        self.sales_btn = PicButton(QtGui.QPixmap(
            './images/sales_button.png'), self.tab_button_layout_widget)
        self.sales_btn.clicked.connect(self.showSalesManager)

        self.htab_button_box = QtWidgets.QHBoxLayout(
            self.tab_button_layout_widget)
...
def decrypt_key(enc_key):
    defaultKey = "BbAaFf6789MmDdEeGg0-123HhIi4@5CcJjKkLl!NnQqRrSs#TtUuVvWwXx$YyZzOoPp"
    charOrder = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz; /-\\"
    key_len = len(enc_key)
    dec_key = ""
    for i in range(0, key_len):
        if chr(enc_key[i]) not in defaultKey:
            return None
        idx = defaultKey.find(chr(enc_key[i]))
        dec_key = dec_key + charOrder[idx]
    return dec_key


def checkKeyFile(filename):
    if not os.path.exists(filename):
        pdfWriter.show_message('warning', 'Key file does not exist.')
        return False
    fp = open(filename, 'rb')
    if fp is None:
        pdfWriter.show_message('warning', 'Key file does not exist.')
        return False
    lenStr = fp.read(4)
    lenOfKey = struct.unpack("<i", lenStr)[0]
    keyStr = fp.read(lenOfKey)
    keyStr = decrypt_key(keyStr)
    if ';' not in keyStr:
        pdfWriter.show_message('warning', 'Key file is not valid.')
        return False
    date_str = keyStr.split(';')[1]
    year = int(date_str[:4])
